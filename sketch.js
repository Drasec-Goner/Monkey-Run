...
function preload()
{
  ...
  dieSound=loadSound("monkeydeath.mp3");
  checkPointSound=loadSound("checkPoint.mp3");
  gameOverSound=loadSound("gameOver.mp3");
  fruitSound = loadSound("fruitcheck.mp3")
}
//600,400
...
function draw()
{
  ...
  //Instructions for playing this game/USER GUIDE
   ...
   fill("black");
   text("1.Press UP Arrow Key for long jump",50,135);
   text("2.Press Space Key to Jump",50,160);
   text("3.Try to collect oranges to get extra stamina",50,190);
   text("4.Collect bananas to score and get stamina",50,220);
   text("5.The game will get progressively difficult as you score points",50,250);
   text("6.Avoid Long Jump unnecessary as it decreases stamina",50,280);
   fill("red");
   textSize(20);
   text("Press Space Key to Start the Game",150,340);
   fill("black");
   textSize(30);
  ...
  else if(gameState===PLAY)
  {
    ...
    else if(keyDown("UP_ARROW")&&monkey.y>450)
    {
...
function food()
{
  ...
  if(frameCount%150===0)
  {
    ...
    banana=createSprite(600,Math.round(random(120,450)),10,10);
    //To add image to banana
...
function bonusFood()
{
  ...
  orange=createSprite(width,Math.round(random(height-400,height-250)),10,10);
